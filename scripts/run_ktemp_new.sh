...
for repeat_ in 300; do
   for ((ens=1; ens<=30; ens+=2)); do
      ...
      sed "s/ENSEMBLE/$ens_odd/g" ktemp.py > kttmp1_${ens_odd}
      sed "s/TRAIN_ITERS/$repeat_/g" kttmp1_${ens_odd} > kttmp2_${ens_odd}
      sed "s/TRAIN_UPDATE/$TRAIN_UP/g" kttmp2_${ens_odd} > $py_file_odd
      
      (
         export CUDA_VISIBLE_DEVICES=1
         python $py_file_odd
         rm -f kttmp*_${ens_odd} $py_file_odd
      ) &
      pid_odd=$!
      ...
      if [ $ens_even -le 30 ]; then
          ...
          sed "s/ENSEMBLE/$ens_even/g" ktemp.py > kttmp1_${ens_even}
          sed "s/TRAIN_ITERS/$repeat_/g" kttmp1_${ens_even} > kttmp2_${ens_even}
          sed "s/TRAIN_UPDATE/$TRAIN_UP/g" kttmp2_${ens_even} > $py_file_even
          
          (
             export CUDA_VISIBLE_DEVICES=0
             python $py_file_even
             rm -f kttmp*_${ens_even} $py_file_even
          ) &
          pid_even=$!
      fi
